...
{
    public record UpdateStrategusSettlementItemsCommand : IMediatorRequest<IList<ItemStack>>
    {
        public int HeroId { get; init; }
        public int SettlementId { get; init; }
        public IList<ItemIdStack> Items { get; init; } = Array.Empty<ItemIdStack>();

        public class Validator : AbstractValidator<UpdateStrategusSettlementItemsCommand>
        {
            public Validator()
            {
        ...
        internal class Handler : IMediatorRequestHandler<UpdateStrategusSettlementItemsCommand, IList<ItemStack>>
        {
            private static readonly ILogger Logger = LoggerFactory.CreateLogger<UpdateStrategusSettlementItemsCommand>();
            ...
            public async Task<Result<IList<ItemStack>>> Handle(UpdateStrategusSettlementItemsCommand req, CancellationToken cancellationToken)
            {
            ...
            private IEnumerable<(Item item, int diff)> ComputeSettlementItemDiff(
                IEnumerable<StrategusSettlementItem> oldSettlementItems,
            ...
            private Result UpdateHeroAndSettlementItems(StrategusHero hero, StrategusSettlement settlement,
                IEnumerable<(Item item, int diff)> settlementItemDiff)
            {
                ...
                foreach (var itemDiff in settlementItemDiff)
                {
                    ...
                    else // Update existing item stack.
                    {
                        ...
                        if (heroItem.Count == 0)
                        {
                            _db.StrategusHeroItems.Remove(heroItem);
                        }
                    }

                    if (settlementItem == null) // If settlement did not have this item before.
                    {
                        settlementItem = new StrategusSettlementItem
                        {
                            Count = itemDiff.diff,
                            Item = itemDiff.item,
                        };
                    ...
                    else // Update existing item stack.
                    {
                        ...
                        {
                            _db.StrategusSettlementItems.Remove(settlementItem);
                        }
                    }
                }
