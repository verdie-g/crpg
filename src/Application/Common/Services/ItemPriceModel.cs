...
/// </summary>
internal class ItemPriceModel
{
    public int ComputeItemPrice(Item item)
    {
        float tier = CalculateTier(item);
        return (int)GetEquipmentPriceFromTier(tier);
    }
    ...
    private static float GetEquipmentPriceFromTier(float tier)
    {
        const float a = 300;
        const float b = 60;
        const float c = 50;
