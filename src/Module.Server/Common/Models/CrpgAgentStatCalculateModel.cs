...
internal class CrpgAgentStatCalculateModel : AgentStatCalculateModel
{
        ...
        int weaponSkill)
    {
        ...
        float weaponClassMultiplier = weapon.WeaponClass switch
        {
            ...
            WeaponClass.Stone => (float)Math.Pow(weapon.ThrustDamage / 20f, 1.5f) * 1f,
            WeaponClass.ThrowingAxe => (float)Math.Pow(weapon.ThrustDamage / 29f, 1.5f) * 1.65f,
            WeaponClass.ThrowingKnife => (float)Math.Pow(weapon.ThrustDamage / 29f, 1.5f) * 1.0f,
            WeaponClass.Javelin => (float)Math.Pow(weapon.ThrustDamage / 35f, 1.5f) * 1.65f,
            _ => 1f,
        };
    ...
    private void UpdateHumanAgentStats(Agent agent, AgentDrivenProperties props)
    {
        ...
        if (equippedItem != null)
        {
            ...
            if (agent.HasMount && !equippedItem.IsRangedWeapon)
            {
                ...
                float swingSpeedFactor = 1f / Math.Max(equippedItem.WeaponLength / 140f, 1f);
