...
internal class CrpgAgentStatCalculateModel : AgentStatCalculateModel
{
    ...
    private void UpdateHumanAgentStats(Agent agent, AgentDrivenProperties props)
    {
        ...
        props.TopSpeedReachDuration = 1.5f * (1f + perceivedWeight / 10f) * (20f / (20f + (float)Math.Pow(athleticsSkill / 120f, 2f)));
        ...
        props.SwingSpeedMultiplier = 0.755f + 0.008f * (float)Math.Pow(itemSkill, 0.65f);
