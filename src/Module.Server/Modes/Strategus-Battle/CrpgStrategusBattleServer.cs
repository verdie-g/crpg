...
internal class CrpgStrategusBattleServer : MissionMultiplayerGameModeBase
{
    ...
    protected override void HandleNewClientAfterLoadingFinished(NetworkCommunicator networkPeer)
    {
        ...
        GameNetwork.WriteMessage(new StrategusBattleMoraleChangeMessage(_morale));
        GameNetwork.EndModuleEventAsServer();
    }

    protected override void HandleEarlyNewClientAfterLoadingFinished(NetworkCommunicator networkPeer)
    {
        networkPeer.AddComponent<StrategusBattleMissionRepresentative>();
    }
    ...
    private void CheckMorales()
    {
        ...
        _StrategusBattleClient.ChangeMorale(_morale);
    }
    ...
    private void TickFlags()
    {
        foreach (var flag in _flags)
        {
            ...
            if (flagOwnerChanged)
            {
                ...
                GameNetwork.BeginBroadcastModuleEvent();
                GameNetwork.WriteMessage(new StrategusBattleCapturePointMessage(flag.FlagIndex, team));
                GameNetwork.EndBroadcastModuleEvent(GameNetwork.EventBroadcastFlags.None);
                _StrategusBattleClient.CaptureFlag(flag, team);
    ...
    private void CheckRemovalOfFlags()
    {
        ...
        _checkFlagRemovalTimer ??= new Timer(Mission.CurrentTime, _StrategusBattleClient.FlagsRemovalTime);
        ...
        if (flagIndexesToRemove.Length > 0) // In case there is only one flag on the map.
        {
            ...
            GameNetwork.BeginBroadcastModuleEvent();
            GameNetwork.WriteMessage(new StrategusBattleFlagsRemovedMessage());
            GameNetwork.EndBroadcastModuleEvent(GameNetwork.EventBroadcastFlags.None);

            _StrategusBattleClient.ChangeNumberOfFlags();
    ...
    private int RemoveFlag(FlagCapturePoint flag)
    {
        ...
        GameNetwork.BeginBroadcastModuleEvent();
        GameNetwork.WriteMessage(new StrategusBattleCapturePointMessage(flag.FlagIndex, null));
        GameNetwork.EndBroadcastModuleEvent(GameNetwork.EventBroadcastFlags.None);
