...
internal class CrpgDTVServer : MissionMultiplayerGameModeBase
{
    ...
    private void CheckMorales()
    {
        ...
        _dtvClient.ChangeMorale(_morale);
    }
    ...
    private void TickFlags()
    {
        foreach (var flag in _flags)
        {
            ...
            if (flagOwnerChanged)
            {
                ...
                GameNetwork.EndBroadcastModuleEvent(GameNetwork.EventBroadcastFlags.None);
                _dtvClient.CaptureFlag(flag, team);
    ...
    private void CheckRemovalOfFlags()
    {
        ...
        _checkFlagRemovalTimer ??= new Timer(Mission.CurrentTime, _dtvClient.FlagsRemovalTime);
        ...
        if (flagIndexesToRemove.Length > 0) // In case there is only one flag on the map.
        {
            ...
            _dtvClient.ChangeNumberOfFlags();
    ...
    private void OnPreRoundEnding()
    {
        ...
        else
        {
            ...
            else if (defenderTeamAlive && attackerTeamAlive)
                {
                if (_morale > 0.0f)
                {
                    roundResult = CaptureTheFlagCaptureResultEnum.AttackersWin;
