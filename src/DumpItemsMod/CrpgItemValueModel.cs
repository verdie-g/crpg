...
namespace Crpg.DumpItemsMod
{
    public class CrpgItemValueModel : DefaultItemValueModel
    {
        ...
        {
            return (horseComponent.IsPackAnimal ? 25f : (0f + 0.6f * (float)horseComponent.Maneuver + (float)horseComponent.Speed + 1.5f * (float)horseComponent.ChargeDamage + 0.1f * (float)horseComponent.HitPoints)) / 13f;
        }
        ...
        private float CalculateRangedWeaponTier(WeaponComponent weaponComponent)
        {
            ...
            return num * ((float)thrustDamage * (float)thrustSpeed * (float)accuracy * 0.00001f);
        }

        private float CalculateShieldTier(WeaponComponent weaponComponent)
        {
            WeaponComponentData weaponComponentData = weaponComponent.Weapons[0];
            return ((float)weaponComponentData.MaxDataValue + 3f * (float)weaponComponentData.BodyArmor + (float)weaponComponentData.ThrustSpeed) / (6f + weaponComponent.Item.Weight) * 0.13f;
        }
        ...
        private float CalculateAmmoTier(WeaponComponent weaponComponent)
        {
            ...
            int num = Math.Max(0, (int)weaponComponentData.MaxDataValue);
        ...
        private float CalculateTierCraftedWeapon(WeaponDesign craftingData)
        {
            ...
            if (num4 > 0 && num2 > 0)
            {
                return 0.4f * (1.25f * (float)num / (float)num2) + 0.6f * ((float)num3 * 1.3f / ((float)num4 + 0.6f));
            }
        ...
        public override int CalculateValue(ItemObject item)
        {
            ...
            {
                num2 = (float)((item.ItemType == ItemObject.ItemTypeEnum.BodyArmor) ? 1.20 : ((item.ItemType == ItemObject.ItemTypeEnum.HandArmor) ? 1.20 : ((item.ItemType == ItemObject.ItemTypeEnum.LegArmor) ? 1.20 : 1.00)));
            }
        ...
        {
            if (item.ItemComponent is ArmorComponent armorComponent)
            {
                return this.CalculateArmorTier(armorComponent);
            }

            if (item.ItemComponent is WeaponComponent weaponComponent)
            {
                return this.CalculateWeaponTier(weaponComponent);
            }

            if (item.ItemComponent is HorseComponent horseComponent)
            {
                return this.CalculateHorseTier(horseComponent);
            }

            if (item.ItemComponent is SaddleComponent saddleComponent)
            {
                return this.CalculateSaddleTier(saddleComponent);
            }
