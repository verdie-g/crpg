...
it.each<[Partial<ClanMemberRole>, boolean]>([
  ...
  expect(canManageApplicationsValidate(role as ClanMemberRole)).toEqual(expectation);
});
...
it.each<[Partial<ClanMemberRole>, boolean]>([
  ...
  expect(canUpdateClanValidate(role as ClanMemberRole)).toEqual(expectation);
});
